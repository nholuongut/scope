...
import (
	...
	billing "github.com/nholuongutworks/billing-client"
	"github.com/nholuongut/scope/app"
	"github.com/nholuongut/scope/app/multitenant"
	...
	"github.com/nholuongutworks/nholuongut/common"
)
...
type flags struct {
	...
	mode                             string
	debug                            bool
	nholuongutEnabled                     bool
	nholuongutHostname                    string
	dryRun                           bool
...
func makeBaseCheckpointFlags() map[string]string {
	...
	return map[string]string{
		// Inconsistent key (using a dash) to match nholuongut Net
...
func setupFlags(flags *flags) {
	...
	flag.BoolVar(&flags.nholuongutEnabled, "nholuongut", true, "Enable nholuongut Net integrations.")
	flag.StringVar(&flags.nholuongutHostname, "nholuongut.hostname", app.DefaultHostname, "Hostname to advertise/lookup in nholuongutDNS")
	...
	flag.StringVar(&flags.probe.token, serviceTokenFlag, "", "Token to authenticate with cloud.nholuongut.works")
	flag.StringVar(&flags.probe.token, probeTokenFlag, "", "Token to authenticate with cloud.nholuongut.works")
	...
	// nholuongut
	flag.StringVar(&flags.probe.nholuongutAddr, "probe.nholuongut.addr", "127.0.0.1:6784", "IP address & port of the nholuongut router")
	flag.StringVar(&flags.probe.nholuongutHostname, "probe.nholuongut.hostname", "", "Hostname to lookup in nholuongutDNS")
	...
	flag.StringVar(&flags.app.nholuongutAddr, "app.nholuongut.addr", app.DefaultnholuongutURL, "Address on which to contact nholuongutDNS")
	flag.StringVar(&flags.app.nholuongutHostname, "app.nholuongut.hostname", "", "Hostname to advertise in nholuongutDNS")
...
func main() {
	...
	if flags.nholuongutHostname != "" {
		if flags.probe.nholuongutHostname == "" {
			flags.probe.nholuongutHostname = flags.nholuongutHostname
		}
		if flags.app.nholuongutHostname == "" {
			flags.app.nholuongutHostname = flags.nholuongutHostname
		}
	}
	flags.probe.nholuongutEnabled = flags.nholuongutEnabled
	flags.app.nholuongutEnabled = flags.nholuongutEnabled
	...
	case "version":
		fmt.Println("nholuongut Scope version", version)
	case "help":
		flag.PrintDefaults()
	default:
