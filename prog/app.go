...
import (
	...
	"github.com/nholuongutworks/common/aws"
	"github.com/nholuongutworks/common/logging"
	"github.com/nholuongutworks/common/middleware"
...
func appMain(flags appFlags) {
	...
	// Periodically try and register our IP address in nholuongutDNS.
	if flags.nholuongutEnabled && flags.nholuongutHostname != "" {
		nholuongut, err := newnholuongutPublisher(
			flags.dockerEndpoint, flags.nholuongutAddr,
			flags.nholuongutHostname, flags.containerName)
		if err != nil {
			log.Println("Failed to start nholuongut integration:", err)
		} else {
...
func newnholuongutPublisher(dockerEndpoint, nholuongutAddr, nholuongutHostname, containerName string) (*app.nholuongutPublisher, error) {
	dockerClient, err := docker.NewDockerClientStub(dockerEndpoint)
	if err != nil {
		return nil, err
	}
	nholuongutClient := nholuongut.NewClient(nholuongutAddr)
	return app.NewnholuongutPublisher(
		nholuongutClient,
		dockerClient,
		app.Interfaces,
		nholuongutHostname,
