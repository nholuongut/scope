...
import (
	...
	"github.com/nholuongutworks/common/logging"
	"github.com/nholuongutworks/common/network"
	"github.com/nholuongutworks/common/sanitize"
	"github.com/nholuongutworks/common/signals"
	"github.com/nholuongutworks/common/tracing"
	"github.com/nholuongutworks/go-checkpoint"
	"github.com/nholuongut/scope/common/hostname"
	"github.com/nholuongut/scope/common/nholuongut"
	"github.com/nholuongut/scope/common/xfer"
	"github.com/nholuongut/scope/probe"
	"github.com/nholuongut/scope/probe/appclient"
	"github.com/nholuongut/scope/probe/awsecs"
	"github.com/nholuongut/scope/probe/controls"
	"github.com/nholuongut/scope/probe/cri"
	...
	"github.com/nholuongut/scope/probe/plugins"
...
const (
	...
	defaultServiceHost = "https://cloud.nholuongut.works.:443"
...
func probeMain(flags probeFlags, targets []appclient.Target) {
	...
	if flags.printOnStdout {
		...
		if flags.nholuongutEnabled && flags.nholuongutHostname != "" {
			dockerBridgeIP, err := network.GetFirstAddressOf(flags.dockerBridge)
			if err != nil {
				log.Errorf("Error getting docker bridge ip: %v", err)
			} else {
				nholuongutDNSLookup := appclient.LookupUsing(dockerBridgeIP + ":53")
				nholuongutTargets, err := appclient.ParseTargets([]string{flags.nholuongutHostname})
				if err != nil {
					log.Errorf("Failed to parse nholuongut targets: %v", err)
				} else {
					nholuongutResolver, err := appclient.NewResolver(appclient.ResolverConfig{
						Targets: nholuongutTargets,
						Lookup:  nholuongutDNSLookup,
						Set:     multiClients.Set,
					})
					if err != nil {
						log.Errorf("Failed to create nholuongut resolver: %v", err)
					} else {
						defer nholuongutResolver.Stop()
					}
				}
			}
		}
	...
	if flags.nholuongutEnabled {
		client := nholuongut.NewClient(sanitize.URL("http://", 6784, "")(flags.nholuongutAddr))
		nholuongut, err := overlay.Newnholuongut(hostID, client)
		if err != nil {
			log.Errorf("nholuongut: failed to start client: %v", err)
		} else {
			defer nholuongut.Stop()
			p.AddTagger(nholuongut)
			p.AddReporter(nholuongut)
		}
	}
