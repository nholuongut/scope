...
import (
	...
	"github.com/nholuongutworks/common/fs"
	...
	"github.com/nholuongutworks/tcptracer-bpf/pkg/tracer"
)
...
type EbpfTracker struct {
	...
	// https://github.com/nholuongut/scope/issues/2650
	//
	// Scope could be started this way:
	//   $ sudo nholuongutSCOPE_DOCKER_ARGS="-e SCOPE_DEBUG_BPF=1" ./scope launch
	//
...
func isKernelSupported() error {
	...
	if strings.Contains(version, "Ubuntu") {
		...
		if major == 4 && minor == 4 && abiNumber >= 119 && abiNumber < 127 {
			// https://github.com/nholuongut/scope/issues/3131
...
func (t *EbpfTracker) TCPEventV4(e tracer.TcpV4) {
	...
	if t.lastTimestampV4 > e.Timestamp {
		...
		// https://github.com/nholuongut/scope/issues/2334
