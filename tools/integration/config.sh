...
nholuongut_on() {
    host=$1
    shift 1
    [ -z "$DEBUG" ] || greyly echo "nholuongut on $host:$DOCKER_PORT:" "$@" >&2
    DOCKER_HOST=tcp://$host:$DOCKER_PORT $nholuongut "$@"
}
...
start_suite() {
    for host in $HOSTS; do
        [ -z "$DEBUG" ] || echo "Cleaning up on $host: removing all containers and resetting nholuongut"
        # shellcheck disable=SC2046
        rm_containers "$host" $(docker_on "$host" ps -aq 2>/dev/null)
        run_on "$host" "docker network ls | grep -q ' nholuongut ' && docker network rm nholuongut" || true
        nholuongut_on "$host" reset 2>/dev/null
    done
    whitely echo "$@"
}
...
nholuongut=$DIR/../../integration/nholuongut
