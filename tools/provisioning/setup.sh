...
function ssh_private_key() {
    ...
    #   $ openssl enc -in /tmp/nholuongutworks_cit_id_rsa -e -aes256 -pass stdin | openssl base64 > /tmp/nholuongutworks_cit_id_rsa.aes.b64
    ...
    #   $ ssh-keygen -t rsa -b 4096 -C "nholuongutworks-cit"
...
function set_up_ssh_private_key() {
    ...
    local ssh_private_key_path="$HOME/.ssh/nholuongutworks_cit_id_rsa"
...
function gcp_credentials() {
    ...
    #   $ openssl enc -in ~/.ssh/nholuongutworks-cit.json -e -aes256 -pass stdin | openssl base64 > /tmp/nholuongutworks-cit.json.aes.b64
    # The below command does the reverse, i.e. base64-decode and AES-decrypt the file, and prints it to stdout.
...
function do_on() {
    ...
    # SSH public key:
    export TF_VAR_do_public_key_path="$HOME/.ssh/nholuongutworks_cit_id_rsa.pub"
    ssh_public_key >"$TF_VAR_do_public_key_path"
    export DIGITALOCEAN_SSH_KEY_NAME="nholuongutworks-cit"
    ...
    export DIGITALOCEAN_TOKEN_NAME="nholuongutworks-cit"
...
function gcp_on() {
    ...
    export TF_VAR_gcp_public_key_path="$HOME/.ssh/nholuongutworks_cit_id_rsa.pub"
    ...
    # JSON credentials:
    export GOOGLE_CREDENTIALS_FILE="$HOME/.ssh/nholuongutworks-cit.json"
    ...
    export TF_VAR_client_ip=$(curl -s -X GET http://checkip.amazonaws.com/)
    export TF_VAR_gcp_project="${PROJECT:-"nholuongut-net-tests"}"
...
function aws_on() {
    ...
    # SSH public key:
    export TF_VAR_aws_public_key_name="nholuongutworks_cit_id_rsa"
...
Examples:
  \$ tf_ansi setup_nholuongut-net_dev
  \$ tf_ansi 1
  \$ tf_ansi 1 -vvv --private-key=~/.ssh/custom_private_key_id_rsa
  \$ tf_ansi setup_nholuongut-kube --extra-vars "docker_version=1.12.6 kubernetes_version=1.5.6"
Available playbooks:
EOF
