...
resource "aws_security_group" "allow_nholuongut" {
  name        = "${var.name}_allow_nholuongut"
  description = "AWS security group to allow communication with nholuongut on AWS EC2 instances (created using Terraform)."

  # Open TCP port for nholuongut:
  ingress {
    from_port   = 12375
    to_port     = 12375
    protocol    = "tcp"
...
resource "aws_instance" "tf_test_vm" {
  ...
  security_groups = [
    ...
    "${aws_security_group.allow_nholuongut.name}",
