...
# List all available versions of nholuongut Net's dependencies:
# - Go
# - Docker
# - Kubernetes
#
...
DEPS = {
    ...
    'docker': {
        ...
        # nholuongut Net only works with Docker from 1.10.0 onwards, so we ignore
    ...
    'kubernetes': {
        ...
        # nholuongut Kube requires Kubernetes 1.4.2+, so we ignore all previous
        # versions:
        'min': '1.4.2',
    }
}
...
def _read_go_version_from_dockerfile():
    # Read Go version from nholuongut/build/Dockerfile
    ...
    with open(dockerfile_path, 'r') as f:
        for line in f:
            ...
            if m:
                return m.group(1)
    raise RuntimeError(
        "Failed to read Go version from nholuongut/build/Dockerfile."
        " You may be running this script from somewhere else than nholuongut/tools."
    )


def _try_set_min_go_version():
    ''' Set the current version of Go used to build nholuongut Net's containers as
    the minimum version. '''
    try:
