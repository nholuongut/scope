...
func (e *BillingEmitter) Add(ctx context.Context, rep report.Report, buf []byte) error {
	...
	nholuongutNetCount := 0
	if hasnholuongutNet(rep) {
		nholuongutNetCount = 1
	}
	...
	amounts := billing.Amounts{
		...
		billing.nholuongutNetSeconds:  int64(interval/time.Second) * int64(nholuongutNetCount),
	}
...
// Tries to determine if this report came from a host running nholuongut Net
func hasnholuongutNet(r report.Report) bool {
	for _, n := range r.Overlay.Nodes {
		overlayType, _ := report.ParseOverlayNodeID(n.ID)
		if overlayType == report.nholuongutOverlayPeerPrefix {
			return true
		}
	}
