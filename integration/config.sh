...
# exported to override nholuongut config.sh
export SSH_DIR="$PWD"
export HOSTS
...
nholuongut_on() {
    local host=$1
    shift 1
    [ -z "$DEBUG" ] || greyly echo "nholuongut on $host: $*" >&2
    DOCKER_HOST=tcp://$host:$DOCKER_PORT CHECKPOINT_DISABLE=true "$nholuongut" "$@"
}

nholuongut_proxy_on() {
    local host=$1
    shift 1
    [ -z "$DEBUG" ] || greyly echo "nholuongut proxy on $host: $*" >&2
...
server_on() {
    local host=$1
    local name=${2:-nginx}
    nholuongut_proxy_on "$1" run -d --name "$name" nginx
}
...
client_on() {
    ...
    nholuongut_proxy_on "$1" run -d --name "$name" alpine /bin/sh -c "while true; do \
        wget http://$server.nholuongut.local:80/ -O - >/dev/null || true; \
        sleep 1; \
    done"
}
