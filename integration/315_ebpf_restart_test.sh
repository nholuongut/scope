...
DOCKER_HOST=tcp://${HOST1}:${DOCKER_PORT} CHECKPOINT_DISABLE=true \
    nholuongutSCOPE_DOCKER_ARGS="-e SCOPE_DEBUG_BPF=1" \
    "${SCOPE}" launch
...
docker_on "$HOST1" exec nholuongutscope sh -c "echo stop > /var/run/scope/debug-bpf"
sleep 5
...
exec 3>&1
assert_raises "docker_on $HOST1 logs nholuongutscope 2>&1 | grep 'ebpf tracker died, restarting it' || (docker_on $HOST1 logs nholuongutscope 2>&3 ; false)"
...
assert_raises "docker_on $HOST1 logs nholuongutscope 2>&1 | grep 'ebpf tracker died again, gently falling back to proc scanning' || (docker_on $HOST1 logs nholuongutscope 2>&3 ; false)"
