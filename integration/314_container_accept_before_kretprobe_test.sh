...
start_suite "Test accept before kretprobe, see https://github.com/nholuongutworks/tcptracer-bpf/issues/10"
...
nholuongut_proxy_on "$HOST1" run -d --name server busybox /bin/sh -c "while true; do \
		date ;
		sleep 1 ;
	done | nc -l -p 8080"
...
nholuongut_proxy_on "$HOST1" run -d --name client busybox /bin/sh -c "ping -c 5 server.nholuongut.local; \
	while true; do \
		date ;
		sleep 1 ;
	done | nc server.nholuongut.local 8080"
