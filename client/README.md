...
- You need at least Node.js 6.9.0 and a running `nholuongutscope` container
...
- You need a running `nholuongutscope` container
- Develop: `make WEBPACK_SERVER_HOST=<dockerhost-ip> client-start` and then open `http://<dockerhost-ip>:4042/`

This will start a webpack-dev-server that serves the UI from the UI build container and proxies API requests to the nholuongutscope container.
