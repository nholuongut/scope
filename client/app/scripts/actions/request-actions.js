...
function updateWebsocketChannel(getState, dispatch, forceRequest) {
  ...
  // Only recreate websocket if url changed or if forced (nholuongut cloud instance reload);
...
function pollTopologies(getState, dispatch, initialPoll = false) {
  // Used to resume polling when navigating between pages in nholuongut Cloud.
