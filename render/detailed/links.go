...
var (
	...
	topologyQueries = map[string]map[string]string{
		...
		report.Service: {
			docker.CPUTotalUsage: `sum(rate(container_cpu_usage_seconds_total{image!="",namespace="{{namespace}}",_nholuongut_pod_name="{{label}}",job="cadvisor",container_name!="POD"}[5m]))`,
			docker.MemoryUsage:   `sum(rate(container_memory_usage_bytes{image!="",namespace="{{namespace}}",_nholuongut_pod_name="{{label}}",job="cadvisor",container_name!="POD"}[5m]))`,
		},
	}
)
